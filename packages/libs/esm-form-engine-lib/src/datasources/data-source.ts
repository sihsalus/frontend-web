import { openmrsFetch, type OpenmrsResource } from '@openmrs/esm-framework/src/internal';
...
export class BaseOpenMRSDataSource implements DataSource<OpenmrsResource> {
  ...
  fetchData(searchTerm: string): Promise<OpenmrsResource[]> {
    return openmrsFetch<ResultsResponse<OpenmrsResource>>(searchTerm ? `${this.url}&q=${searchTerm}` : this.url).then(
      ({ data }) => data.results ?? [],
    );
  }
  ...
  fetchSingleItem(uuid: string): Promise<OpenmrsResource | null> {
    ...
    return openmrsFetch<OpenmrsResource>(apiUrl).then(({ data }) => data);
  }
