...
export const ProgramEnrollmentSubmissionAction: PostSubmissionAction = {
  applyAction: async function ({ patient, encounters, sessionMode }, config) {
    ...
    const resolvedConfig = (config ?? {}) as ProgramEnrollmentConfig;

    if (!encounter) {
      return;
    }

    const encounterLocation = getResourceUuid(encounter.location);
    ...
    const programUuid = isStringValue(resolvedConfig.programUuid) ? resolvedConfig.programUuid : undefined;
    ...
    const enrollmentDate = encounter.obs?.find((item) =>
      item.formFieldPath?.includes(resolvedConfig.enrollmentDate ?? ''),
    )?.value;
    const completionDate = encounter.obs?.find((item) =>
      item.formFieldPath?.includes(resolvedConfig.completionDate ?? ''),
    )?.value;
    ...
    let payload: PatientProgramPayload = {
      patient: patient.id,
      program: programUuid,
      dateEnrolled: isStringValue(enrollmentDate) ? enrollmentDate : dayjs().format(),
    ...
    if (resolvedConfig.completionDate) {
      if (!isStringValue(completionDate)) {
    ...
    try {
      const response = await saveProgramEnrollment(payload, abortController);
      showSnackbar({
        kind: 'success',
        title: getSnackTitle(t, response.data),
        isLowContrast: true,
      });
    } catch (error) {
      showSnackbar({
...
function getSnackTitle(
  t: (key: string, defaultValue: string, options?: Omit<TOptions, 'ns' | 'defaultValue'>) => string,
  response: PatientProgram,
): string {
  if (response.dateCompleted) {
...
function updateTimeToNow(dateString: string): string {
