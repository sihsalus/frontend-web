...
test.describe('Critical User Journeys', () => {
  ...
  test('Patient Search & View', async ({ page }) => {
    ...
    if (await searchInput.isVisible().catch(() => false)) {
      ...
      const patientResult = page.locator('[data-testid="patient-result"], .patient-result, button:has-text("10009")').first();
  ...
  test('Form Submission - Vitals', async ({ page }) => {
    ...
    const vitalButton = page.locator('button:has-text("Vitals"), button:has-text("vital"), [data-testid*="vital"]').first();
    ...
    if (await vitalButton.isVisible().catch(() => false)) {
      ...
      // 2. Submit form
      const submitButton = page.locator('button:has-text("Save"), button:has-text("Submit"), button:contains("Guardar")').first();
